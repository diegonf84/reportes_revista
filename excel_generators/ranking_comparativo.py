...
def create_excel_ranking_comparativo_completo(csv_path: str, output_path: str, period: str) -> None:
    """
    Genera archivo Excel completo para el reporte 'ranking_comparativo'.
...
def crear_hoja_ranking(ws, df, tipos_disponibles):
    ...
    for tipo in tipos_disponibles:
        ...
        # Convertir M.T.P.P. -> MTTP para mostrar
        nombre_tipo = "MTTP" if tipo == "M.T.P.P." else tipo
        
        # Título del tipo
        cell_tipo = ws.cell(row=current_row, column=1, value=nombre_tipo)
        ...
        # Total del tipo
        totales = calcular_totales_tipo(tipo_data)
...
def crear_hoja_detalle(ws, df):
    """Crea la hoja 'base_detalle' con toda la información"""
    ...
    # Headers
    headers = ["Tipo", "ENTIDAD", "Primas emitidas", "Variación %", "Primas anterior"]
    columns_map = ['tipo_cia', 'nombre_corto', 'primas_emitidas', 'variacion', 'primas_anterior']
    ...
    for _, row_data in datos_ordenados.iterrows():
        for col_idx, csv_col in enumerate(columns_map, 1):
            ...
            # Convertir M.T.P.P. -> MTTP para mostrar
            if csv_col == 'tipo_cia' and value == 'M.T.P.P.':
                value = 'MTTP'
...
def generate_ranking_comparativo_excel(period: str, csv_dir: str = None, output_dir: str = None) -> str:
    """
    Función de conveniencia para generar el Excel de ranking comparativo.
    ...
    output_path = os.path.join(period_dir, f"{period}_ranking_comparativo.xlsx")
    
    create_excel_ranking_comparativo_completo(csv_path, output_path, period)
    return output_path

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='Genera Excel para ranking comparativo')
    ...
    try:
        excel_path = generate_ranking_comparativo_excel(args.period)
